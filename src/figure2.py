...
def make_ood_source(size=256, char="7", img_path=None):
    ...
    if img_path is not None:
        ...
        ax.text(0.5, 0.46, char, fontsize=60, fontweight="bold",
...
def render_figure(src, clean, samples, obs_list, noises, out):
    ...
    gs = fig.add_gridspec(2, ncol, left=L, right=R, top=TOP, bottom=BOT,
                          wspace=0.0, hspace=0.0)   # no gutters -> filmstrip
    ...
    show(0, 0, src); show(1, 0, clean)
    for k, s in enumerate(noises):
        show(0, k + 1, samples[k])
        a = show(1, k + 1, obs_list[k])
        a.text(0.93, 0.06, fr"$\sigma_\mathcal{{N}}={s:g}$", transform=a.transAxes,
    ...
    fig.text(x1, TOP + 0.015, r"Samples from the posterior $p(x\mid y)$",
             ha="center", fontsize=17)
    fig.text(L - 0.045, BOT + 0.75 * (TOP - BOT), "Background\nsource (x)",
             va="center", ha="center", rotation=90, fontsize=13)
    fig.text(L - 0.045, BOT + 0.25 * (TOP - BOT), "Distorted\nimage (y)",
             va="center", ha="center", rotation=90, fontsize=13)
    xa = L + (R - L) / ncol
    fig.add_artist(FancyArrowPatch((xa, BOT - 0.05), (R, BOT - 0.05),
                   transform=fig.transFigure, arrowstyle="-|>",
                   mutation_scale=18, lw=1.4, color="black"))
    fig.text((xa + R) / 2, BOT - 0.085, "Data with increasing levels of noise",
...
def main():
    ...
    p.add_argument("--source_img", type=str, default=None,
    ...
    for k, s in enumerate(noises):
        ...
        if part.exists():
            ...
            samples.append(d["x"]); obs_list.append(d["y"])
        ...
        x = posterior_sample(model, sim, y=y, sigma_y=s, steps=args.steps,
                             n_samples=1, source_size=src.shape[-1], image_pool=2)
        torch.save({"x": x[0, 0].cpu(), "y": y.cpu(), "sigma": s}, part)
        samples.append(x[0, 0].cpu()); obs_list.append(y.cpu())
